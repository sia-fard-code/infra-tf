...
module "alb" {
  ...
  vpc_prv_subnet_cidr_blocks = { "BLUE" = data.terraform_remote_state.common.outputs.vpc_prv_subnet_cidr_blue }
...
module "web_asg" {
  ...
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-web"]
...
module "api_asg" {
  ...
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-api"]
  deployment_map          = [local.color]
  instance_type           = "m5.large"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "admin_asg" {
  ...
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-admin-app"]
  deployment_map          = [local.color]
...
module "risk_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-risk-service"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "streaming_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-streaming-service"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "system_events_rest_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-system-events-rest"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "system_events_consumer_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-system-events-consumer"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "external_integration_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-external-integration"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "vitals_rest_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-vitals-rest"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "vitals_data_capture_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-vitals-data-capture"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "legacy_data_capture_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-legacy-data-capture"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "gateway_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-gateway"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "activity_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-activity-service"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "open_rest_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-open-rest"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "status_scheduler_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-status-scheduler"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "hifi_download_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-hifi-download"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "telehealth_service_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-telehealth-service"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "patient_settings_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-patient-settings"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
...
module "notification_service_asg" {
  ...
  source                  = "../modules/asg-lt"
  ami                     = module.variables.ubuntu_amis
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}${var.msn_prefix}-notification-service"]
  deployment_map          = [local.color]
  instance_type           = "t3.medium"
  key_name                = data.terraform_remote_state.common.outputs.app_server_key_name
