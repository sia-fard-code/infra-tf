locals {
  ...
  records_prv = terraform.workspace == "PROD-US" ? [
   {
      name    = "app"
      type    = "A"
      alias   = {
        name    = module.alb_common[0].api_dns_name_prv
        zone_id = module.alb_common[0].api_zone_id_prv
      }
    },
] : [
   {
      ...
      type    = "A"
      alias   = {
        name    = module.alb_common[0].db_dns_name_prv
        zone_id = module.alb_common[0].db_zone_id_prv
      }
    },
    {
      name    = "app"
 ...
 records_pub = [
   {
      ...
      type    = "CNAME"
      ttl            = 60
      records        = ["www-g.${local.fqdn}"]
    },
    {
      name    = "app"
      type    = "CNAME"
      ttl            = 60
      records        = ["app-g.${local.fqdn}"]
    },
    {
      name    = "admin"
      type    = "CNAME"
      ttl            = 60
      records        = ["admin-g.${local.fqdn}"]
    },
 ]
...
data "aws_acm_certificate" "cert" {
  count    = terraform.workspace == "PROD-US" ? 1 : 0
  domain   = "*.${local.fqdn}"
  statuses = ["ISSUED"]
}
...
module "vpc" {
  ...
  azs_b              = "${local.region}${var.second_region_map[local.environment]}"
...
module "alb_common" {
  ...
  count                      = contains(var.new_b_g_env, terraform.workspace) ? 1 : 0
  env                        = local.environment
  msn_svc_enabled            = local.msn_svc_enabled
  msn_open_enabled           = local.msn_open_enabled
  msn_th_enabled           = local.msn_th_enabled
...
//  domain_arn                 = terraform.workspace == "PROD-US" ? data.aws_acm_certificate.cert[0].arn : module.external-certificate[0].acm_certificate_arn
  domain_arn                 = terraform.workspace == "PROD-US" ? data.aws_acm_certificate.cert[0].arn : module.external-certificate.acm_certificate_arn
...
module "msn_route53" {
  ...
  records = local.msn_svc_enabled || local.msn_open_enabled ? concat(
    [
    {
      ...
      type    = "A"
      alias   = {
        name    = module.alb_common[0].open_api_dns_name
        zone_id = module.alb_common[0].open_api_zone_id
      }
    }
    ],
    local.records_prv,
  ...
  depends_on = [module.alb_common]
}

module "msn_pub_route53" {
  source  = "../modules/records"
  count   = contains(var.new_b_g_env, terraform.workspace) && !contains(var.dr_env, terraform.workspace) ? 1 : 0
...
#  count                   = terraform.workspace == "PROD-US" ? 0 : 1
  ...
  aliases = contains(var.dr_env, terraform.workspace) == true ? {
    prv_db = {
  ...
  } : contains(var.msn_open_env, terraform.workspace) == true ? {
    pub_wide = {
      alias           = "*.${local.fqdn}."
...
module "efs-api" {
  ...
  vpc_id          = module.vpc.vpc_dc_id
  subnets         = [module.vpc.vpc_dc_private_subnets[0], module.vpc.vpc_dc_private_subnets[1]]
  security_groups = [module.sg.alb_api_sg_id, module.sg.alb_hifi_download_sg_id]
  zone_id         = aws_route53_zone.msn_private_zone.zone_id
  dns_name        = "efs-api-${lower(local.environment)}"
}

resource "aws_s3_bucket" "msk_bucket" {
  bucket = "${lower(local.environment)}-open-msk-broker-logs"
  acl    = "private"
}

module "msk-cluster" {
  count                   = contains(var.msk_enable, terraform.workspace)? 1 : 0
  source  = "angelabad/msk-cluster/aws"

  cluster_name    = "${upper(local.environment)}-open-msk"
  ...
  s3_logs_prefix = "logs/${lower(local.environment)}-open-"
...
/*
  lifecycle {
    ignore_changes = [
        broker_node_group_info, current_version
    ]
  }
*/
}
...
module "bastion" {
  ...
  region                      = local.region
  allowed_cidr                = ["50.59.0.128/25", "52.25.47.70/32", "64.222.252.131/32", "64.58.153.2/32"]
...
module "web_asg" {
  ...
  name                    = ["${local.environment}-${var.index_to_deployment_map[0]}-msn-web-scale", "${local.environment}-${var.index_to_deployment_map[1]}-msn-web-scale"]
...
resource "aws_launch_template" "mongo" {
  ...
  lifecycle {
    ignore_changes = [
      tags, image_id, security_group_names, user_data, description, block_device_mappings, tag_specifications, latest_version,
    ]
  }
}
